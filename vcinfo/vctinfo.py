...
class VctAlarmTrade():
    ...
    # create table db (vctalarm_pump table).
    def createPumpMarketDb(self):
        try:
            conn = sqlite3.connect(TARGET_DB)
            try:
                sqlText = 'select * from  vctalarm_pump limit 1'
                comm.executeTxDB(conn, sqlText)
            except Exception as e:
                logging.error(' vctalarm_pump table init : %s' % e)
                sqlText = 'create table vctalarm_pump (id integer primary key autoincrement, check_time text , market text , market_name text, trade_price text, rate text,  crt_dttm text)'
                comm.executeTxDB(conn, sqlText)
                conn.commit()
        except Exception as e:
            logging.error(' Exception : %s' % e)
    ...
    def setPumpCoins(self, check_time, market, market_name, trade_price, rate, crt_dttm):
        try:
            ...
            sqlText = "delete from vctalarm_pump where check_time = '"+check_time+"' and  market  = '"+market+"'"
            comm.executeTxDB(conn, sqlText)

            sqlText = 'insert into vctalarm_pump  (check_time, market, market_name, trade_price, rate, crt_dttm)'
            sqlText += ' values (?, ?, ?, ?, ?, ?)'
    ...
    def loadMarketCandlesDaysSaveToDb(self):
        ...
        try:
            ...
            sqlText = 'insert into vctalarm_candles_days  (market, candle_date_time_utc, candle_date_time_kst, opening_price, high_price, low_price, trade_price, timestamp, candle_acc_trade_price, candle_acc_trade_volume, prev_closing_price, change_price,  change_rate, converted_trade_price)'
            sqlText += ' values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,  ?, ?)'
    ...
    # load market candles weeks info save to db (vctalarm_candles_week table).
    def loadMarketCandlesWeeksSaveToDb(self):
        logger.warn('loadMarketCandlesWeeksSaveToDb start')
        try:
            conn = sqlite3.connect(TARGET_DB)
            try:
                sqlText = 'drop table vctalarm_candles_weeks'
                comm.executeTxDB(conn, sqlText)
            except Exception as e:
                logging.error(' Exception : %s' % e)

            sqlText = 'create table vctalarm_candles_weeks (market text, candle_date_time_utc text, candle_date_time_kst text, opening_price real, high_price real, low_price real, trade_price real, timestamp integer, candle_acc_trade_price real, candle_acc_trade_volume real, first_day_of_period  text)'
            comm.executeTxDB(conn, sqlText)
            ...
            sqlText = 'insert into vctalarm_candles_weeks  (market, candle_date_time_utc, candle_date_time_kst, opening_price, high_price, low_price, trade_price, timestamp, candle_acc_trade_price, candle_acc_trade_volume, first_day_of_period)'
            sqlText += ' values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ...
    # load market candles months info save to db (vctalarm_candles_week table).
    def loadMarketCandlesMonthsSaveToDb(self):
        logger.warn('loadMarketCandlesMonthsSaveToDb start')
        try:
            conn = sqlite3.connect(TARGET_DB)
            try:
                sqlText = 'drop table vctalarm_candles_months'
                comm.executeTxDB(conn, sqlText)
            except Exception as e:
                logging.error(' Exception : %s' % e)

            sqlText = 'create table vctalarm_candles_months (market text, candle_date_time_utc text, candle_date_time_kst text, opening_price real, high_price real, low_price real, trade_price real, timestamp integer, candle_acc_trade_price real, candle_acc_trade_volume real, first_day_of_period  text)'
            comm.executeTxDB(conn, sqlText)
            ...
            sqlText = 'insert into vctalarm_candles_months  (market, candle_date_time_utc, candle_date_time_kst, opening_price, high_price, low_price, trade_price, timestamp, candle_acc_trade_price, candle_acc_trade_volume, first_day_of_period)'
            sqlText += ' values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ...
    def queryContinueGrowsMarkets(self, date_type="M", whereCondition=None, recent_count = "33"):
        ...
        target_table = ""
        if date_type == "M":
            target_table = " vctalarm_candles_months "
        elif date_type == "W":
            target_table = " vctalarm_candles_weeks "
        else:
            target_table = " vctalarm_candles_days "
